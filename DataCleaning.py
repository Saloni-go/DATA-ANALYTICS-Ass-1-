...
def fill_missing_with_mean(df, col_name, rows=None):
    ...
    # Always fill only the detected rows
    df.loc[rows.index, col_name] = mean_val
    print(f"Filled {len(rows)} rows in '{col_name}' with mean: {mean_val}")
...
def fill_missing_with_median(df, col_name, rows=None):
    ...
    print(f"Filled {len(rows)} rows in '{col_name}' with median: {median_val}")
...
def remove_duplicates(df, col_name=None):
    ...
    if not duplicates.empty:
        ...
        print(duplicates)
        df = interactive_next_step(df, duplicates, col_name, message=f"duplicates in '{col_name}'")
    else:
